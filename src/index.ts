...
program
  .name('ai-code-review')
  .description('AI code review CLI: reviews diffs between two commits with RAG + LLM')
  ...
  .option('--llm-base-url <url>', 'OpenAI-compatible base URL', process.env.LLM_BASE_URL)
  .option('--llm-api-key <key>', 'LLM API key', process.env.LLM_API_KEY)
  .option('--llm-model <model>', 'LLM model name', process.env.LLM_MODEL || 'gpt-4o-mini')
  .option('--format <fmt>', 'Output format: json|md|both', 'both')
  .option('--output <path>', 'Write output to file (for md or json)')
  .option('--max-tokens <n>', 'Max tokens for LLM', (v) => Number.parseInt(v, 10), 1200)
  .option('--temperature <n>', 'LLM temperature', (v) => Number.parseFloat(v), 0.2)
...
program.action(async (opts) => {
  ...
  const result = await reviewRepositoryDiff({
    ...
    rawRagConfig: parsed.ragUrl ? { baseUrl: parsed.ragUrl, apiKey: parsed.ragKey } : undefined,
    rawLlmConfig:
