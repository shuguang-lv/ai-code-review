...
export const chunkParsedDiff = (
	...
	for (const file of parsed.files) {
		...
		for (let i = 0; i < file.hunks.length; i++) {
			...
			if (
				!current ||
				current.tokenEstimate + est > maxTokensPerChunk ||
...
const estimateHunkTokens = (
	lines: Array<{ lineNumber: number; content: string }>,
) => {
	const lengthCost = lines.reduce(
		(acc, l) => acc + Math.ceil(l.content.length / 4),
		0,
	); // ~4 chars/token
...
export const summarizeChunkForPrompt = (
	parsed: ParsedDiff,
	chunk: DiffChunk,
): {
	filePath: string;
	...
	if (!file) return { filePath: chunk.filePath, hunks: [] };
	const hSummaries = chunk.hunks.map((idx) => {
		...
		return {
			...
			preview: h.addedLines
				.slice(0, 8)
				.map((l) => ({ n: l.lineNumber, c: l.content.slice(0, 240) })),
		};
	});
	return { filePath: file.path, hunks: hSummaries };
};
